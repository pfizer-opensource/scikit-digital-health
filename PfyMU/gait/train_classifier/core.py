...
__all__ = ['load_datasets', 'load_datasets_v2']
...
def load_datasets_v2(paths, device_location, goal_fs=100.0, acc_mag=True, window_length=3.0, window_step=0.5, signal_function=None):
    """
    Load standardized datasets into memory
    ...
    paths : str, Path, array_like
        Path to a dataset or an array-like of strings for multiple datasets
    device_location : str
        Body location of the device ex. wrist, lumbar, ankle etc.
    goal_fs : float, optional
        Desired sampling frequency. Will interpolate up/down depending on the sampling frequency of the
        provided datasets. Default is 100.0
    acc_mag : bool, optional
                ...
                for activity in f.keys():
                    ...
                    for trial in f[activity].keys():
                        ...
                        if fs != goal_fs:
                            f_intrp = interp1d(
                                np.arange(0, n/fs, 1/fs)[:n],
                                f[activity][trial][device_location]['Accelerometer'],
                                axis=0,
                                bounds_error=False,
                                fill_value='extrapolate'
                            )
                            ...
                            tmp = f[activity][trial][device_location]['Accelerometer'][()]
