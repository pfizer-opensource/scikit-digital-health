...
class Metrics:
    def __init__(self, short_win_length=5.0, metrics='hfen+'):
        ...
        - enmoa: Euclidean Norm of the acceleration, minus 1g. Additionally, negative values are clipped to 0.
    ...
    def _compute(self, acceleration, fs):
        ...
        metric_1, metric_2, ... : numpy.ndarray
            Arrays of the compute metrics, taking the mean value over non-overlapping windows of length
            `short_win_length`. Returned in the order of `metrics`, as seperate arrays
        """
        ret = {}
        for metr in self.metrics:
            if metr.lower() == 'hfen+':
                ...
                ax = metr.lower()[-1]
                # take the mean of windows, after computing the angle of the rolling median with window_step = 1
