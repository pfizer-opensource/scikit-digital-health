...
def test_preprocessing(t_and_x):
    ...
    proc = PreprocessGaitBout(correct_orientation=True, filter_cutoff=20.0, filter_order=4)
...
def test_vertical_cwt_gait_events(t_and_x):
    ...
    res = proc.predict(time=t, accel_filt=x, fs=50.0, v_axis=0, v_axis_sign=1, mean_step_freq=1.0)

    assert allclose(res['initial_contacts'], [13, 63, 113, 163, 213])  # peaks in the sine wave
    assert allclose(res['final_contacts'], [24, 76, 126, 176, 228])  # peaks in the sine derivative
...
def test_ap_cwt_gait_events(t_and_x):
    ...
    res = proc.predict(time=t, accel_filt=x, fs=50.0, v_axis=0, v_axis_sign=1, mean_step_freq=1.0, ap_axis=1, ap_axis_sign=1)
...
def test_strides_qc_static():
    ...
    proc = CreateStridesAndQc(
        max_stride_time=2.25,
        loading_factor=0.2
    )
    res = proc.predict(time=t, fs=50.0, initial_contacts=ic, final_contacts=fc, mean_step_freq=0.95)
    ...
    res2 = proc.predict(time=t, fs=50., initial_contacts=ic, final_contacts=fc, mean_step_freq=0.95)
    ...
    res3 = proc.predict(time=t, fs=50., initial_contacts=ic, final_contacts=fc, mean_step_freq=0.95)
...
def test_turns(gait_input_gyro):
    ...
    proc = TurnDetection()
    res = proc.predict(time=t, accel=acc, gyro=gyro, fs=50., qc_initial_contacts=ic, qc_final_contacts=fc)

    truth = array([0] * 17 + [1, 2, 2, 2, 2, 2, 2] + [0] * 8 + [1, 2, 2, 2, 2] + [0] * 8)
