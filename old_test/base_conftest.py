...
class BaseProcessTester:
    ...
    sample_data_keys = ["file", "time", "accel", "gyro", "temperature"]
    ...
    def test(self, get_sample_data, get_truth_data):
        ...
        truth_data = get_truth_data(
            self.truth_data_file, self.truth_data_keys, self.truth_suffix
        )
    ...
    def dict_allclose(self, pred, truth, keys):
        for key in keys:
            if key == "time":
                ...
                assert allclose(
                    ptime, ttime, atol=self.atol_time
                ...
                if isinstance(truth[key], dict):
                    ...
                    assert allclose(
                        pred[key], truth[key], atol=self.atol, equal_nan=True
                    ), f"{self.process._name} test for value ({key}) not close to truth"
