...
! --------------------------------------------------------------------
! SUBROUTINE  dpqsort_2d
!     Perform a double pivot quicksort on a special 2D array, and move the
!     indices accordingly as well
!
...
! --------------------------------------------------------------------
subroutine dpqsort_2d(m, order, x, idx)
    implicit none
    integer(8), intent(in) :: m, order
    real(8), intent(inout) :: x(m, order)
    integer(8), intent(inout) :: idx(m, order)
!f2py intent(hide) :: m, order
    integer(8) :: i, j, jj, itemp(m)
    real(8) :: temp(m)
...
! --------------------------------------------------------------------
recursive subroutine dpqsort(n, x, idx)
    implicit none
    integer(8), intent(in) :: n
    real(8), intent(inout) :: x(n)
    integer(8), intent(inout) :: idx(n)
!f2py intent(hide) :: n
    integer(8) :: i, j, k, l, g, itemp, ip1, ip2
    real(8) :: temp, p1, p2
    ...
    if (l .gt. 2) then
        ...
        idx(1) = idx(l - 1)
        idx(l - 1) = ip1
        call dpqsort(l-2, x(1:l-2), idx(1:l-2))
    end if
    call dpqsort(g-l, x(l:g-1), idx(l:g-1))
    if (g .lt. n) then
        x(n) = x(g)
        x(g) = p2
        idx(n) = idx(g)
        idx(g) = ip2
        call dpqsort(n-g, x(g+1:n), idx(g+1:n))
    end if   
end subroutine
...
!     x(n)  : real(8), array of values to sort (inplace)
! --------------------------------------------------------------------
recursive subroutine dpqsort_no_idx(n, x)
    implicit none
    integer(8), intent(in) :: n
    real(8), intent(inout) :: x(n)
!f2py intent(hide) :: n
    ...
    if (l .gt. 2) then
        x(1) = x(l - 1)
        x(l - 1) = p1
        call dpqsort_no_idx(l-2, x(1:l-2))
    end if
    call dpqsort_no_idx(g-l, x(l:g-1))
    if (g .lt. n) then
        x(n) = x(g)
        x(g) = p2
        call dpqsort_no_idx(n-g, x(g+1:n))
    end if   
end subroutine