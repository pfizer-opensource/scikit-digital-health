...
class TestEqualAverageDurationThreshold:
    def test(self, act_results):
        e = EqualAverageDurationThreshold(
            min_threshold=0.0001, max_threshold=0.1, skip_threshold=0.0001, state="wake"
        )
        ...
        assert allclose(
            act_results["wake duration equal avg duration [min]"], 1.9439252
        )
...
class TestActivitySignalFeatures:
    def test(self, act_results):
        ...
        assert allclose(act_results["wake 15min signal entropy"], -0.905934)
        assert allclose(act_results["wake 15min sample entropy"], 0.0050216387)
        assert allclose(act_results["wake 15min permutation entropy"], 0.99477370)
        assert allclose(act_results["wake 15min power spectral sum"], 1.00000005)
        assert allclose(act_results["wake 15min spectral flatness"], -3.08924230)
        assert allclose(act_results["wake 15min spectral entropy"], 0.88117488)
        assert allclose(act_results["wake 15min sparc"], -15.867399)
