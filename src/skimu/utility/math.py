...
def rolling_median(a, w_len, skip=1, pad=False, axis=-1):
    r"""
    Compute the rolling mean.
    ...
    -------
    rmed : numpy.ndarray
        Rolling median. Note that if the rolling axis is not the last axis, then the result
        will *not* be c-contiguous.
    ...
    where `n` is the length of the rolling axis. Computation efficiency does not decrease for
    ...
    rmed = _extensions.rolling_median(x, w_len)
