...
def _update_date_results(
    ...
    results["Day Start"][day_n] = start_dt.strftime("%Y-%m-%d %H:%M:%S.%f")
    results["Day End"][day_n] = datetime.utcfromtimestamp(time[day_stop_idx]).strftime("%Y-%m-%d %H:%M:%S.%f")
...
class ActivityLevelClassification(BaseProcess):
    ...
    def predict(self, *, time, accel, fs=None, wear=None, **kwargs):
        ...
        res = {
            ...
            "Day Start": full(n_, "", dtype="U26"),
            "Day End": full(n_, "", dtype="U26"),
